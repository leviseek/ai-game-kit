...
function recordingListHandle(): {
    ...
    const handle: IFairyGuiListHandle<unknown> = {
        ...
        setItemClick: (handler) => {
            state.itemClick = handler as (index: number, item: { heroId: string; deployed: boolean }) => void;
        },
        refresh: () => {},
    };
    return {
...
describe.skipIf(!AUTO_BATTLE_ASSEMBLY_EXISTS)("Auto-battle lineup editor presenter", () => {
