...
describe("Auto-battle fixture contract file", () => {
    test("declares createAutoBattleFixture without cc or fgui imports", () => {
        expect(existsSync(AUTO_BATTLE_ASSEMBLY_FILE), "assets/game_auto_battle/assembly.ts not implemented yet (task 3.1)").toBe(true);
        ...
        expect(source).toMatch(/\bexport\s+(?:function|const)\s+createAutoBattleFixture\b/);
...
describe.skipIf(!AUTO_BATTLE_ASSEMBLY_EXISTS)("Auto-battle fixture composition", () => {
        ...
        expect(() =>
            createAutoBattleFixture({
                configContent: configContent({
                    ally: [unit("a0", "Tank"), unit("a1", "Mage"), unit("a2", "Priest"), unit("a3", "Extra"), unit("a4", "Extra2"), unit("a5", "Extra3"), unit("a6", "Extra4")],
                }),
            }),
        ).toThrow();
        expect(() =>
            createAutoBattleFixture({
                configContent: configContent({
                    enemy: [unit("e0", "Grunt"), unit("e1", "Raider"), unit("e2", "Shaman"), unit("e3", "Extra"), unit("e4", "Extra2"), unit("e5", "Extra3"), unit("e6", "Extra4")],
                }),
            }),
        ).toThrow();
    });
    ...
    test("restart resets the battle, its event log and is idempotent", async () => {
        ...
        expect(fixture.battle.events.map((e) => e.type)).toEqual(["restart", "round-start"]);

        // 幂等：再次重开状态保持一致
        fixture.battle.restart();
        state = fixture.battle.state;
        expect(state.round).toBe(1);
        expect(state.units.every((u) => u.hp === u.maxHp && u.energy === 0)).toBe(true);
        expect(fixture.battle.events.map((e) => e.type)).toEqual(["restart", "round-start"]);
...
describe("Auto-battle framework boundary", () => {
    test("the framework layer declares no auto-battle business models", () => {
        ...
        const modelPattern = /\b(?:interface|class|type|enum)\s+(?:AutoBattle\w*)\b/;
