...
import type { ApplicationVisibility, ApplicationVisibilityState, DeviceInfo, PlatformStorage } from "../../../assets/framework/contracts/platform/Platform";
import type { TimeSource } from "../../../assets/framework/contracts/time/TimeSource";
import type { InputContextId, InputEvent, InputMapping, InputSample, InputSource, InputSourceId } from "../../../assets/framework/contracts/input/Input";
...
import type { ConfigKey, ConfigReadType, ConfigTable } from "../../../assets/framework/contracts/config/Config";
...
type ExpectedLifecycleHook = (context: ApplicationContext) => void | Promise<void>;
...
type _ApplicationStateIsClosedUnion = Expect<Equal<ApplicationState, "created" | "initializing" | "running" | "paused" | "stopping" | "disposed">>;
...
type _ApplicationStateIsLifecycleState = Expect<Equal<Application["state"], ApplicationState>>;
...
type _ApplicationContextHasNoApplicationIdentity = Expect<NeverKey<ApplicationContext, "application" | "app" | "identity" | "owner">>;

type _ModuleShape = Expect<Equal<keyof Module, "id" | "dependencies" | LifecycleHookName>>;
type _ModuleIdIsString = Expect<Equal<Module["id"], string>>;
type _ModuleIdIsReadonly = Expect<IsReadonlyKey<Module, "id">>;
type _DependenciesAreReadonlyIds = Expect<Equal<Module["dependencies"], readonly string[]>>;
type _DependenciesDoNotContainModules = Expect<Equal<Extract<Module["dependencies"][number], Module>, never>>;
type _ModulePhaseIsClosedUnion = Expect<Equal<ModulePhase, LifecycleHookName>>;
type _ModuleRuntimeStateIsClosedUnion = Expect<Equal<ModuleRuntimeState, "registered" | "initialized" | "started" | "paused" | "stopped" | "disposed">>;
type _ModuleHasNoManagerPattern = Expect<NeverKey<Module, "create" | "register" | "configure" | "manager" | "builder">>;
type _ModuleHasNoEventBus = Expect<NeverKey<Module, "on" | "emit" | "subscribe" | "publish" | "events">>;
...
type _LifecycleHooksUseApplicationContext = Expect<
    Equal<
        {
            [Hook in LifecycleHookName]: Equal<NonNullable<Module[Hook]>, ExpectedLifecycleHook>;
        }[LifecycleHookName],
        true
    >
>;

type _LoggerShape = Expect<Equal<keyof Logger, "debug" | "info" | "warn" | "error" | "child">>;
type _LogLevelIsClosedUnion = Expect<Equal<LogLevel, "debug" | "info" | "warn" | "error">>;
type _LogContextIsRecord = Expect<Equal<LogContext, Readonly<Record<string, unknown>>>>;
...
type _PlatformShape = Expect<Equal<keyof ApplicationVisibility, "state" | "onVisibilityChange" | "setVisibility">>;
type _ApplicationVisibilityStateIsClosedUnion = Expect<Equal<ApplicationVisibilityState, "foreground" | "background">>;
type _ApplicationVisibilityStateIsReadonly = Expect<IsReadonlyKey<ApplicationVisibility, "state">>;
type _PlatformStorageShape = Expect<Equal<keyof PlatformStorage, "delete" | "get" | "set">>;
...
>;
type _DeviceInfoShape = Expect<Equal<keyof DeviceInfo, "language" | "model" | "platform">>;
type _DeviceInfoIsReadonly = Expect<IsReadonlyKey<DeviceInfo, "platform"> & IsReadonlyKey<DeviceInfo, "model"> & IsReadonlyKey<DeviceInfo, "language">>;
type _TimeSourceShape = Expect<Equal<keyof TimeSource, "now">>;
type _TimeSourceReturnsNumber = Expect<Equal<TimeSource["now"], () => number>>;
...
type _InputEventShape = Expect<Equal<keyof InputEvent, "sourceId" | "pressed" | "value">>;
type _InputEventSourceIdIsReadonly = Expect<IsReadonlyKey<InputEvent, "sourceId">>;
type _InputEventValueIsOptional = Expect<IsOptionalKey<InputEvent, "value">>;
type _InputSampleShape = Expect<Equal<keyof InputSample<TestAction>, "action" | "pressed" | "value" | "timestamp">>;
type _InputSampleActionIsGeneric = Expect<Equal<InputSample<TestAction>["action"], TestAction>>;
type _InputSampleIsReadonly = Expect<IsReadonlyKey<InputSample<TestAction>, "action"> & IsReadonlyKey<InputSample<TestAction>, "pressed"> & IsReadonlyKey<InputSample<TestAction>, "value">>;
type _InputMappingIsRecordOfActions = Expect<Equal<InputMapping<TestAction>, Readonly<Record<string, TestAction>>>>;
type _InputSourceShape = Expect<Equal<keyof InputSource, "id" | "subscribe">>;
type _InputSourceSubscribeReturnsUnsubscribe = Expect<Equal<InputSource["subscribe"], (listener: (event: InputEvent) => void) => () => void>>;
...
type _InputMapperSetMappingsIsTyped = Expect<Equal<InputMapper<TestAction>["setMappings"], (mappings: Readonly<Record<InputContextId, InputMapping<TestAction>>>) => void>>;
type _InputMapperReplaceSourceIsTyped = Expect<Equal<InputMapper<TestAction>["replaceSource"], (source: InputSource) => void>>;
...
type _ConfigTableShape = Expect<Equal<keyof ConfigTable, "read" | "snapshot">>;
type _ConfigTableReadIsOverloadedWithDefault = Expect<
    ...
    >
>;
type _ConfigReadTypeParsesUnknownToDeclared = Expect<Equal<ConfigReadType<number>["parse"], (key: ConfigKey, raw: unknown) => number>>;
type _ConfigTableSnapshotReturnsReadonly = Expect<Equal<ConfigTable["snapshot"], () => Readonly<Record<ConfigKey, unknown>>>>;
