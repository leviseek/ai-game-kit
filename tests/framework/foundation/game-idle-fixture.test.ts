...
const assemblyFile = resolve(projectRoot, "assets/game_idle/assembly.ts");
...
async function loadCreateIdleFixture(): Promise<CreateIdleFixture> {
    ...
    )) as { createIdleFixture: CreateIdleFixture };
...
describe("Idle fixture contract file", () => {
    test("declares createIdleFixture without cc or fgui imports", () => {
        expect(
            ...
            "assets/game_idle/assembly.ts not implemented yet (task 4.2)",
        ).toBe(true);
...
describe.skipIf(!assemblyExists)(
    "Idle fixture composition capabilities",
    () => {
...
describe("Idle fixture framework boundary", () => {
    test("the framework layer declares no offline earnings or growth models", () => {
        ...
        const modelPattern =
            /\b(?:interface|class|type|enum)\s+(?:(?:Idle|Offline|Earn|Growth|Income|Clicker|Gold)\w*)\b/;
