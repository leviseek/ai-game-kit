...
const assemblyFile = resolve(projectRoot, "assets/game_tycoon/assembly.ts");
...
async function loadCreateTycoonFixture(): Promise<CreateTycoonFixture> {
    ...
    )) as { createTycoonFixture: CreateTycoonFixture };
...
describe("Tycoon fixture contract file", () => {
    test("declares createTycoonFixture without cc or fgui imports", () => {
        expect(
            ...
            "assets/game_tycoon/assembly.ts not implemented yet (task 5.2)",
        ).toBe(true);
        ...
        expect(source).toMatch(
            /\bexport\s+(?:function|const)\s+createTycoonFixture\b/,
        );
...
describe.skipIf(!assemblyExists)(
    "Tycoon fixture composition capabilities",
    () => {
        ...
        test("layered UI presents the economy state through two layer routes", async () => {
            ...
            expect(fixture.navigator.pages.map((p) => p.layer)).toEqual([
                "normal",
                "popup",
            ]);
        ...
        test("failRollback does not disturb the fixture's own capabilities", async () => {
            ...
            expect(fixture.navigator.open("tycoon/hub", { layer: "normal" }).ok).toBe(
                true,
            );
...
describe("Tycoon fixture framework boundary", () => {
    test("the framework layer declares no production chain or economy models", () => {
        ...
        const modelPattern =
            /\b(?:interface|class|type|enum)\s+(?:(?:Tycoon|Economy|Production|Factory|Inventory|Goods|Sell|Cash|Workshop|Product)\w*)\b/;
