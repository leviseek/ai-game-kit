...
- [ ] 1.1 先编写状态机测试，覆盖合法转换、拒绝非法事件、未知事件不破坏状态、进入/退出钩子顺序、钩子失败回滚与状态一致、reset 与 dispose（释放后拒绝事件、重复释放幂等）。
- [ ] 1.2 实现 `core/fsm` 下的纯 TypeScript 状态机（声明式转移表 + 轻量运行器），失败经错误报告回调隔离，返回同步幂等 `DisposeHandle`，使 1.1 通过且不依赖 Cocos。
