...
- [ ] 1.1 先编写加载协调器测试，覆盖并发加载去重、加载失败传播、取消等待者和不同作用域共享同一底层加载结果。
- [ ] 1.2 实现引擎无关的资源 handle（资源标识、归属 Bundle、加载状态、解析结果）与加载协调器，使 1.1 的测试通过且不依赖 Cocos。
- [ ] 1.3 补充依赖边界检查，验证 `framework/core/resource` 不导入 `cc`、资源契约不依赖具体实现。
