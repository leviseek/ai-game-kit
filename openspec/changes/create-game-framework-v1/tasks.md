...
- [x] 8.2 建立回合制卡牌组合夹具，验证可控模拟时间、状态机、配置、输入和 UI 可以协作，且框架中没有卡组/回合规则。（由 change `implement-game-composite-fixtures-v1` 3.x 完成：`assets/game_card/assembly.ts` 导出 `createCardFixture`（注入 clock/configContent/inputSource），`battle.ts` 经 `createStateMachine` 表达 player/enemy/over 回合流、enemy 阶段经时钟超时自动回 player，`config.ts` 不可变配置驱动数值、`input.ts` 联动出牌、`ui.ts` route；`game-card-fixture.test.ts` 11 pass 锁定确定性回合，负向断言确认框架层无卡组/回合规则。）
...
- [x] 8.4 建立模拟经营组合夹具，验证调度、配置、存档和分层 UI 可以协作，且生产链和经济模型位于游戏层。（由 change `implement-game-composite-fixtures-v1` 5.x 完成：`assets/game_tycoon/assembly.ts` 导出 `createTycoonFixture`（注入 clock/configContent/storage），`scheduler.ts` 被动调度驱动生产任务、`production.ts` 生产推进与经济结算、`save.ts` 版本化存档、`ui.ts` normal（hub）/popup（factory）分层 route；`game-tycoon-fixture.test.ts` 12 pass，负向断言确认生产链/经济模型在游戏层。）
