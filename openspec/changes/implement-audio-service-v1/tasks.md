...
- [ ] 1.1 先编写分组测试：覆盖 music/sfx/ui 分组独立音量与静音状态、跨分组修改互不影响。
- [ ] 1.2 先编写音量与静音测试：覆盖非法音量拒绝并保留原值、静音不破坏音量设定、取消静音恢复原音量。
- [ ] 1.3 先编写播放/停止/切歌测试：覆盖按资源播放、切歌停止前一首、停止未播放音频无副作用、暂停/恢复。
- [ ] 1.4 先编写作用域停止测试：覆盖作用域释放停止其启动的全部音频、不影响其他作用域播放。
- [ ] 1.5 先编写降级测试：覆盖后端不可用时所有操作无副作用成功返回且服务可查询为降级状态。
