...
协调器维护资源键到共享加载期的映射。本 Change 资源键为 Bundle + 路径的单文件模型，但键结构 MUST 为 FairyGUI package 预留类型维度（如 `kind: "asset" | "fairygui-package"`），使后续 package 可复用协调器、作用域与引用计数，只是键空间不同。首个请求触发底层加载，后续请求复用同一加载期；加载完成后分发结果给所有等待者。失败只分发一次并通知全部等待者，保留 cause 与资源标识。等待者的取消（所属作用域释放或显式取消）只移除其自身回调，不影响其他等待者。loader 同步抛异常与返回 rejected promise 走同一条失败路径（同步异常先转为 failed 终态再创建 handle），避免 entry 永久停留在 loading 或向调用方裸抛无资源标识的错误。

**Entry 生命周期决策（衔接 task 2/5 的契约）：** 终态 entry（ready/failed）由协调器实例持有直到该实例销毁，本阶段不实现驱逐或失效接口。后果是：同 key 重复 `load` 恒返回缓存终态（含已释放资源引用或同一失败）。task 2 引用计数归零要释放资源、task 5 重试要重新加载时，必须显式处理此缓存语义（例如新增 `invalidate(key)`，或由上层重建协调器实例）；在此之前协调器不对外暴露失效能力，避免在错误假设上实现。

**理由：** 并发去重与失败广播是资源系统的核心可测试行为；共享加载期同时天然支持多作用域共享同一底层资源。终态缓存让晚加入等待者立即获得结果，但把失效策略显式推迟给有真实释放/重试需求的阶段。
