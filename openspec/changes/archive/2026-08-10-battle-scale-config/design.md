...
- 真源划分：`position`（front/mid/back）仍是目标选择优先级的唯一语义；`side+index`（index 即**队内**逻辑槽位序号 0..N-1）是实例化顺序与目标同排稳定次序的身份。二者由配置直接给定，无相互推导，避免双真源。
- **渲染寻址（重要澄清）**：模型层 `index` 是队内序号，而绑定节点名 `unit_{全局索引}` 是**全局索引**（先己方后敌方拼接，全局索引 = side 偏移 + 队内 slotIndex）。渲染层必须显式做偏移转换（己方偏移 0、敌方偏移 N），否则 6v6 时敌我两侧队内 index 均为 0..5 会拼出相同节点名冲突。
- 渲染映射 `slotToXY(side, slotIndex, teamSize) → {x, y}` 为**单向纯函数**：由槽位序推导屏幕坐标（敌左、己右），不反向回写逻辑。坐标只服务表现层，逻辑层不消费 xy。
- 落点：`models.ts` 注释将 `index` 语义正式化为"队内逻辑槽位序号 0..N-1"；`view/view.ts` 新增映射表纯函数 + 全局索引偏移转换。
...
- 6 槽/侧在 1280×720 下的精确垂直排布（槽间距/日志区位置）需 fgui-designer 实测后定稿，属实现期微调，不影响 specs 与任务拆解。定量参考：现有槽 y=60/170/280（间距 110px）、组高 92px，组内元素（name 28 + hp 20 + bar 20 + energy 14）共需约 82px；日志区 `txt_log` y=520、按钮区约 y=376/448/484。6 槽沿用 110px 间距会与日志区重叠（末槽底 610+92=702），可用区 60..520 内每槽约 73px 低于组内所需 82px——需压缩间距/字号/条高或重排日志区，fgui-designer 据此评估。
