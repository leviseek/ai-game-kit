...
命中反馈特效 SHALL 是叠加在 state 全量渲染之上的增量动画：特效播放不改变战斗状态、不产生新事件、不改变事件序列与终局结果；动画结束后目标节点姿态（坐标/透明度）回到 state 快照对应的值，无状态漂移。

#### Scenario: 特效不进入逻辑层

- **WHEN** 战斗推进产生攻击/技能事件的同时播放命中反馈特效
- **THEN** 战斗状态与事件序列不受特效影响，同一对局在有无特效渲染下回放一致（确定性不回归）

#### Scenario: 动画终态回到 state 姿态

- **WHEN** 飘字/闪白/抖动动画结束
- **THEN** 目标节点坐标与透明度回到 state 快照对应的姿态，不残留动画位移或透明度
