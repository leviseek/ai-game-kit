...
/** 项目就绪 = Creator 在运行且编程系统产物新鲜（避免仅凭进程存在误判）。 */
export function isCreatorReady(projectName: string, maxAgeMs = 60000): boolean {
  return isCreatorRunning() && isProgrammingFresh(maxAgeMs);
}
