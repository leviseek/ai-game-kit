...
export function run(argv: readonly string[]): number {
    ...
    if (hasStructuralErrors(issues)) {
        // manifest 结构问题会污染结论，先报结构错误并拒绝给出 check 结果
        printIssues(issues);
        console.error(`[ai-sync:check] manifest 结构错误，check 结论不可信，先修复 registry`);
        return 1;
    }

    const expected = expectedFiles(sync, expandAssets(manifest));
