    ...
    return defineArchitectureConfig({
        hierarchy: {
            root: group("repository", [
                group("source", [
                    group("framework", [
                        group("framework-core", ["src/framework/{core,shared}/**"]),
                    ]),
                    group("game", ["src/game/**"]),
                ]),
            ]),
...
describe("buildHierarchyView", () => {
    test("builds L0-L5 hierarchy, metadata stats, and unclassified files", () => {
        ...
        const declarations = [
            declaration("class:clock", files[0]!, "GameClock"),
            declaration("method:tick", files[0]!, "GameClock::tick"),
            declaration("class:play", files[1]!, "PlaySession"),
        ];
        ...
        const gameDirectory = view.groups.find((item) =>
            item.parentId === "game" && item.metadata?.level === 3);
        const clockFile = view.groups.find((item) =>
            item.metadata?.filePath === "src/framework/core/clock.ts");
        const playFile = view.groups.find((item) =>
            item.metadata?.filePath === "src/game/play.ts");
        ...
        expect(root?.metadata).toEqual(expect.objectContaining({
            childCount: 2,
            fileCount: 4,
            symbolCount: 3,
            testCount: 1,
        }));
        expect(core?.metadata).toEqual(expect.objectContaining({
            fileCount: 1,
            symbolCount: 2,
            testCount: 0,
        }));
