...
import { branch, defineArchitectureConfig, flow, group, lifecycle, phase, symbol } from "../lib/config/builders";
...
function makeGateway(nodes: readonly CodeGraphNode[]): CodeGraphGateway {
    ...
    const impacts = new Map<string, readonly string[]>([["AppRoot::start", ["RuntimeProbe"]]]);
...
const config = defineArchitectureConfig({
    ...
    dataFlows: [
        flow("close-dialog", [
            ...
            { id: "projection", anchors: [symbol("projectCloseDialog", "assets/samples/game_fui_demo/store.ts"), symbol("CloseDialog::onState", "assets/samples/game_fui_demo/view/CloseDialog.ts")] },
        ]),
    ],
    resources: [
...
describe("semantic views", () => {
    ...
    test("does not compress configured paths across unresolved anchors", async () => {
        ...
        expect(result.nodes.map((item) => item.qualifiedName)).toEqual(["Application::start", "AppRoot::start"]);
    ...
    test("builds startup phases and branches without inventing cross-branch edges", async () => {
        ...
        const fakeEdge = view.edges.find((edge) => edge.from === "node:Application::start" && edge.to === "node:createBootFlow::launch");

        expect(view.type).toBe("startup");
        expect(view.nodes.find((item) => item.qualifiedName === "assembleApp")?.metadata).toEqual(expect.objectContaining({ phase: "assembly" }));
        expect(view.nodes.find((item) => item.qualifiedName === "Application::start")?.metadata).toEqual(expect.objectContaining({ branch: "application" }));
        expect(view.nodes.find((item) => item.qualifiedName === "createBootFlow::launch")?.metadata).toEqual(expect.objectContaining({ branch: "presentation" }));
    ...
    test("builds data flow lanes with direction metadata and declared UI-to-store edge", async () => {
        ...
        const declaredEdge = view.edges.find((edge) => edge.from === "node:CloseDialog::_handleConfirm" && edge.to === "node:closeDialogReducer");

        expect(view.type).toBe("data-flow");
        expect(view.nodes.find((item) => item.qualifiedName === "CloseDialog::bind")?.metadata).toEqual(expect.objectContaining({ lane: "view-input", direction: "source" }));
        expect(view.nodes.find((item) => item.qualifiedName === "CloseDialog::onState")?.metadata).toEqual(expect.objectContaining({ lane: "projection", direction: "sink" }));
    ...
    test("builds call view with incoming, outgoing, affected and test roles", async () => {
        const view = await buildCallView(gateway, [symbol("Application::start", "assets/framework/application/Application.ts"), symbol("AppRoot::start", "assets/boot/AppRoot.ts")]);

        expect(view.type).toBe("calls");
        expect(view.nodes.find((item) => item.qualifiedName === "StartFlow.test")?.metadata).toEqual(expect.objectContaining({ role: "test" }));
        expect(view.nodes.find((item) => item.label === "launch")?.metadata).toEqual(expect.objectContaining({ role: "outgoing" }));
        expect(view.nodes.find((item) => item.qualifiedName === "RuntimeProbe")?.metadata).toEqual(expect.objectContaining({ role: "affected" }));
        expect(view.nodes.find((item) => item.qualifiedName === "Application::start")?.metadata).toEqual(expect.objectContaining({ role: "incoming" }));
    });
    ...
    test("builds resource lifecycle levels with owner, scope and state", async () => {
        ...
        expect(view.nodes.find((item) => item.qualifiedName === "UiHost::loadPackage")?.metadata).toEqual(
            expect.objectContaining({ level: 0, owner: "global-ui-package", scope: "global-ui-package", state: "enter" }),
        );
        expect(view.nodes.find((item) => item.qualifiedName === "UiHost::release")?.metadata).toEqual(
            expect.objectContaining({ level: 1, owner: "global-ui-package", scope: "global-ui-package", state: "exit" }),
        );
        expect(view.nodes.find((item) => item.qualifiedName === "createSceneFlow::switchTo")?.metadata).toEqual(expect.objectContaining({ owner: "scene-flow", scope: "scene-flow", state: "active" }));
    });
});
