...
describe("repository architecture contract", () => {
    test("resolves configured anchors and covers repository hierarchy", async () => {
        ...
        if (status.worktreeMismatch !== undefined && status.worktreeMismatch !== null) {
            throw new Error(`CodeGraph worktree mismatch: index=${status.worktreeMismatch.indexRoot}, worktree=${status.worktreeMismatch.worktreeRoot}`);
        }
        ...
        expect(startupNames).toEqual(expect.arrayContaining(["AppRoot::onLoad", "assembleApp", "createBootFlow::launch", "createSceneFlow::switchTo"]));
        ...
        const ownersByFile = hierarchy.groups.filter((group) => typeof group.metadata?.filePath === "string").map((group) => [group.metadata?.filePath, group.metadata?.ownerGroupId] as const);
        const duplicateOwners = ownersByFile.filter(([filePath], index) => ownersByFile.findIndex(([other]) => other === filePath) !== index);
        expect(duplicateOwners).toEqual([]);
        expect(hierarchy.diagnostics.filter((item) => item.severity === "error" && item.message.includes("multiple hierarchy groups"))).toEqual([]);
    }, 60_000);
});
...
function fileOwner(hierarchy: Awaited<ReturnType<ArchitectureAnalyzer["buildSnapshot"]>>["views"]["hierarchy"], filePath: string): unknown {
