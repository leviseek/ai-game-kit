...
写入流程：先在临时键写入完整新值并读回校验，校验通过后保留当前正式值到备份键，再一次性替换正式键，最后清理临时键（异常路径由 try/finally 清理，失败不残留临时键）。临时键/备份键以 `%tmp`/`%bak` 后缀派生：`encodeURIComponent` 编码输出中 `%` 恒为大写 `%XX` 序列，小写后缀不可能出现在任何编码正式键中，故辅助键与正式键空间严格不相交（`.tmp`/`.bak` 因 `.` 未被编码会与合法键冲突，不可用）。写入中断时正式键要么保持完整旧值、要么得到完整新值，不产生半写入；备份键提供可恢复回退。
...
**权衡：** 适配器不转义调用方键，后缀方案以调用方键名不以 `%tmp`/`%bak` 结尾为前提（versioned-storage 编码键天然满足）；同值重复写不产生替换、不创建备份，备份存在性依赖跨值写入历史。
**未采用方案：** 依赖平台事务（多数 localStorage 后端无事务）；扩展契约加原子方法（见决策 1）；普通 `.tmp`/`.bak` 后缀（与编码键空间冲突，静默覆盖删除其他存档）。
...
`core/storage/SaveCoordinator.ts` 提供引擎无关协调器：订阅 `ApplicationVisibility`，在触发状态（默认 `background`，对应暂停与退出）变化时调度保存；同一时刻至多一个保存在途（串行化），执行期间新触发标记 pending，当前保存完成后收敛到最后一次有效状态（合并写），避免并发交错覆盖与丢失最后一次有效状态。单次保存失败经 `onError` 回调报告（缺省 `console.error`，对齐 `ScopedEventChannel` 缺省模式），不产生未处理拒绝，后续触发仍可收敛落盘。

**理由：** Cocos 生命周期事件可能密集触发；串行化避免并发交错写入，窗口合并避免写放大，最终持久化最后一次有效状态。协调器位于 `core/storage` 仅依赖 `contracts/platform`，不依赖 `cc`，可在测试与任意平台复用。
**未采用方案：** 各平台自行实现保存触发与串行化（重复机制，破坏可测试性）；保存失败静默吞掉（丢失最后一次有效状态且不可诊断）。
...
- 备份键占用额外键空间，恢复路径负责清理；`%tmp`/`%bak` 后缀与编码正式键空间严格不相交，不与版本化存储键冲突。
