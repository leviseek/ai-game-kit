...
**理由：** 导航统一负责生命周期与返回策略（总计划设计决策 7），全局一致避免每个页面各自实现"防重复打开"，降低组合爆炸；三选一覆盖 RPG 单例页面、弹窗队列与 HUD 等常见需求。`focus-existing` 只保证页面在其层级内最高且被激活，但不允许破坏层级不变量（例如 popup 打开时聚焦底层 normal 页面不得把它弹到 popup 之上）。
