 ...
 * headless Chrome + CDP 采集验证（对齐 runUiSmoke）。构建产物已含 `game`
 * 场景（main Bundle 已注册），单向冒烟 startup → game 安全；回切 startup 会
 * 实例化第二个 AppRoot，本序列只做单向。
 */
export async function runSceneFlowSmoke(context: SceneSmokeContext): Promise<void> {
    ...
    report("initial-can-unload-ui", context.canUnload("ui"));

    // 2. 预加载：ui 被流转作用域持有（不可卸载）
    let preloadHolds = false;
    try {
        await context.preload("game", { bundle: "ui", paths: ["placeholder"] });
        preloadHolds = !context.canUnload("ui");
        report("preload", true);
    ...
    try {
        ...
        releaseLoop = context.canUnload("ui");
    ...
    try {
        ...
        report("switch-holds-ui", !context.canUnload("ui"));
    } catch (error) {
