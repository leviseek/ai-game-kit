import { defineFuiViewBinding, type FuiViewBindingRegistrar, type IStore } from "../../framework";
 ...
 * effect 由 Feature assembly 参数注入。View 依赖类型只含 IStore 与 facade
...
/** Feature handle：暴露 Feature 级 IStore；dispose 先注销 registration 再释放 IStore，幂等。 */
export interface CloseDialogFeature {
    readonly store: IStore<CloseDialogState, CloseDialogAction>;
    dispose(): void;
}

/**
 * 装配 CloseDialog Feature（品类 IModule 形态的唯一装配入口）：创建 Feature 级
 * IStore 与 facade，向组合根 registrar 注册「URL → CloseDialog → binder」。
 *
 * 所有权边界：IStore 归 Feature（页面关闭不影响）；页面 scope 只拥有 View 订阅等
 * 页面局部句柄（View.bind 内部 __own 订阅，随页面 dispose 逆序释放）。
 * `dispose()` 先注销 registration 再 dispose IStore，重复调用幂等。
 */
