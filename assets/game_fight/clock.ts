 ...
 * 实现框架 TimeSource 契约，供输入采样时间戳与战斗帧推进共用同一时间基准。
 * 框架根入口不导出 SimulationClock（public-boundary 白名单），
 * 故夹具层自实现最小可控时钟，保证战斗帧推进可经 advance 独立驱动。
 */
export interface FightClock extends TimeSource {
 ...
 */
export function createFightClockModule(clock: FightClock): Module {
  return {
    id: "fight.clock",
    dependencies: [],
    start: () => {
