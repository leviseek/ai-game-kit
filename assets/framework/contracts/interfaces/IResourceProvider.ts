...
import type { IResourceScope } from "./IResourceScope";
...
export interface IResourceProvider {
     ...
     */
    load<T = unknown>(bundle: string, path: string): IResourceHandle<T>;
     ...
     */
    loadPackage<T = unknown>(bundle: string, path: string): IResourceHandle<T>;

    /**
     * 预加载：与 load 同形，发起加载并返回 handle。预加载的消费与释放语义
     * 由后续 SceneFlow 阶段定义，本契约只锁定其存在性与签名。
     */
    preload<T = unknown>(bundle: string, path: string): IResourceHandle<T>;
