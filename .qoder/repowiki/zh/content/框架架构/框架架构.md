...
## 更新摘要
**所做更改**
- 新增了有限状态机（FSM）组件的详细文档
- 更新了核心组件部分，包含FSM组件说明
- 更新了架构总览图，展示FSM在核心基础设施层中的位置
- 添加了FSM组件的详细分析章节
- 更新了依赖关系分析，体现FSM与其他核心组件的关系
...
## 简介
本架构文档面向 AI Game Kit 框架，聚焦高层设计、架构模式与系统边界。框架采用"应用-模块"生命周期管理为核心，通过契约化接口解耦平台适配、事件与调度等横切能力，并以类型化错误与作用域事件通道保障可维护性与稳定性。**新增的有限状态机（FSM）组件**为框架提供了强大的状态管理能力，使开发者能够轻松实现复杂的状态转换逻辑。框架不强制统一玩法模型，允许业务层按特性选择 OOP/ECS/状态机/命令/行为树等实现方式；同时严格划分框架与游戏的职责边界，确保框架稳定、游戏快速演进。
...
- 基础设施：core 提供事件通道、调度器、时间源、错误基类以及**有限状态机**等通用能力。
...
**新增** 有限状态机组件为框架提供了完整的状态管理能力，包括类型安全的状态转换、钩子函数支持和错误处理机制。
...
- 基础设施层：core 提供事件、调度、时间、错误、**有限状态机**等通用能力。
...
**更新** 架构图已添加FSM组件，显示其作为核心基础设施的一部分，与应用层和其他核心组件并列。
...
**更新** 依赖关系图已添加FSM组件，显示其作为core模块的子模块，与其他核心组件并列。
...
**新增** 有限状态机组件经过精心设计，确保高性能和低内存占用：
- 状态转换使用哈希表实现O(1)查找
- 钩子函数仅在需要时执行，避免不必要的开销
- 错误处理采用轻量级机制，不影响正常路径性能
- 内存占用固定，不随状态数量线性增长
...
**新增** 有限状态机故障排查要点：
- 状态转换失败：检查状态转换表中是否正确定义了从当前状态到目标状态的转换
- 钩子函数异常：确认 onExit/onEnter 钩子函数没有抛出未捕获的异常
- 重入问题：避免在钩子函数中再次调用 send() 方法
- 资源泄漏：确保在 onExit 钩子中正确清理资源
...
## 结论
AI Game Kit 框架以"应用-模块"生命周期为核心，通过契约化接口与类型化错误、作用域事件通道、**有限状态机**等横切能力，构建了高内聚、低耦合的可扩展架构。**新增的FSM组件**进一步增强了框架的状态管理能力，使开发者能够轻松实现复杂的业务逻辑。框架与游戏边界清晰，混合玩法架构灵活适配多类型游戏。通过严格的依赖拓扑、幂等状态机与完善的诊断体系，框架在保证稳定性的同时，为业务快速迭代提供了坚实基础。

**更新** 有限状态机的加入使框架具备了更强大的状态管理能力，为复杂游戏逻辑的实现提供了坚实的基础设施支持。
...
- **状态机集成**：FSM组件可直接在业务模块中使用，无需额外配置。

**新增** 有限状态机组件的部署要求：
- 无需额外的运行时依赖
- 可与现有模块系统无缝集成
- 支持与其他核心组件（事件、调度、错误处理）协同工作
...
**更新** 系统上下文图已添加FSM组件，显示其在整个系统中的作用以及与业务模块的交互关系。
